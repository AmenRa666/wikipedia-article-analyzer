  ...
  Article.find({}, (err, docs) => {
    if (err) console.log(err);
      ...
      for (var i = 0; i < docs.length; i++) {
        ...
        if (docs[i].qualityClass == 1 && stubClassCount > 67) {
          docs.splice(docs.indexOf(docs[i]), 1)
          i--
        }
        else if (docs[i].qualityClass == 2 && startClassCount > 67) {
          docs.splice(docs.indexOf(docs[i]), 1)
          i--
        }
        else if (docs[i].qualityClass == 3 && cClassCount > 67) {
          docs.splice(docs.indexOf(docs[i]), 1)
          i--
        }
        else if (docs[i].qualityClass == 4 && bClassCount > 67) {
          docs.splice(docs.indexOf(docs[i]), 1)
          i--
        }
        else if (docs[i].qualityClass == 5 && gaClassCount > 66) {
          docs.splice(docs.indexOf(docs[i]), 1)
          i--
        }
        else if (docs[i].qualityClass == 6 && aClassCount > 66) {
          docs.splice(docs.indexOf(docs[i]), 1)
          i--
        }
      }
      ...
      fs.writeFile('../datasets/MH_FA_NON-FA_Dataset.csv', csv, function(err) {
        if (err) throw err;
